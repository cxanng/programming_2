...
std::string combine_info(const Student s) {
    return s.student_number + ";" + s.user_id + ";" + s.name + ";" + s.phone_number + ";"
            + s.email + ";" + s.skype;
}
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::map< std::string, Student* >::iterator iter = student_numbers.find(student_num);
            ...
            getline(std::cin, new_num);
            std::cout << "\n";
            if ((is_valid_phone_number(new_num))) {
                student_numbers[student_num]->phone_number = new_num;
                for(auto pair: user_ids) {
                    if(pair.second->student_number == student_num) {
                        pair.second->phone_number = new_num;
                    }
                }
            }
            ...
            for (auto pair : user_ids) {
                ...
                file_object << combine_info(*s) << std::endl;
            }
